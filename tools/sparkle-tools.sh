...
# generate_appcast) and prints the directory holding them. Other scripts run
# this and use its output; nothing here is needed to build or run Neon, only to
# publish an update.
...
if [ ! -x "${EXTRACT_DIR}/bin/generate_appcast" ] \
  ...
  if [ ! -f "${ARCHIVE_PATH}" ]; then
    command -v gh >/dev/null 2>&1 || {
      echo "The 'gh' CLI is required to download Sparkle's tools. brew install gh" >&2
      exit 1
    }
